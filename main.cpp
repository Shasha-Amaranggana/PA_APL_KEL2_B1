...
int scrollAkun(vector<Akun> &akun, string judulMenu) {
    ...
    while (true) {
        ...
        for (size_t i = 0; i < indexUser.size(); i++) {
            if (i == pilih)
                {cout << "                                                    " << BG_PUTIH << HITAM << BOLD << akun[indexUser[i]].username << RESET << endl;}
            else {
                cout << DIM << "                                                    " << akun[indexUser[i]].username << RESET << endl;}}
...
void lihatDataDiri(vector<Akun> &akun, int index_login) {
    string spasi = string(50, ' '), passwordSensor = string(akun[index_login].password.length(), '*');
    string field[10] = {
    ...
    system("cls"); judul_subjudul("INFORMASI DATA DIRI");
...
void editStatusPelanggan(vector<Akun> &akun) {
    ...
    while (true) {
        ...
        if (hasil == -1) {
            ...
            // AKTIFKAN
            if (pilih == 0) {
                if (akun[pilihan].status_akun == "Aktif" || akun[pilihan].status_akun == "aktif") {
                    tampilPeringatan(30, "Akun sudah aktif!");}
                else {
                    akun[pilihan].status_akun = "Aktif";
                    saveAkun(akun);
                    tampilPesan(30, "Akun berhasil diaktifkan!");}
                break;}

            // NONAKTIFKAN
            else if (pilih == 1){
                if (akun[pilihan].status_akun == "Nonaktif" || akun[pilihan].status_akun == "nonaktif") {
                    tampilPeringatan(30, "Akun sudah nonaktif!");}
                else {
                    akun[pilihan].status_akun = "Nonaktif";
                    saveAkun(akun);
                    tampilPesan(30, "Akun berhasil dinonaktifkan!");}
                break;}
...
void pesananBelumDibayar(vector<Order> &order, vector<Akun> &akun, int index_login) {
    ...
    lihatPesananUser("Belum Bayar", order, akun[index_login].id_user);
    ...
    if (tolower(pil) == 'y') {
        ...
        for (size_t i = 0; i < order.size(); i++) {
            if (order[i].id_user == id_user && order[i].id_order == id_bayar && order[i].status_order == "Belum Bayar") {
...
void pesananDiproses(vector<Order> &order, string id_user) {
    ...
    lihatPesananUser("Diproses", order, id_user);
    system("pause");
}
...
void editSaldo(vector<Akun> &akun, int index_login) {
    ...
    cout << "\n     ====================================" << endl;
    cout << "            TOP UP / EDIT SALDO          " << endl;
    cout << "     ====================================" << endl;

    cout << "\n     Saldo Saat Ini : Rp"
         << akun[index_login].saldo << endl;

    try {
        cout << "     Masukkan nominal saldo : Rp";
        cin >> tambahSaldo;

        if (cin.fail() || tambahSaldo <= 0) {
            throw "Nominal saldo harus berupa angka dan lebih dari 0!";
        }

        akun[index_login].saldo += tambahSaldo;
        ...
        tampilPesan(35, "Top Up Berhasil!");
    }
